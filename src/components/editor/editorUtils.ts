...
export const getFileLanguage = (filename: string) => {
  ...
  switch (ext) {
    ...
    case 'md':
      return 'markdown';
    default:
      // Handle special cases for Flutter-related files
      if (filename.includes('flutter') || filename.includes('pubspec')) {
        // pubspec files are YAML
        if (filename.includes('pubspec')) {
          return 'yaml';
        }
        // Other Flutter-related files default to dart
        return 'dart';
      }
      // For any other file, return a valid language or plaintext
      return 'plaintext';
  }
};